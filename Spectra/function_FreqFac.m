function[f_w] = function_FreqFac(Freq,Buoy,Method)
%FUNCTION_FREQFAC determines the friction factor based on a specified
% frequency, buoy, and method; and a plot is created of the frequency factor
% curve as a function of frequency. 
%
% Notes:
%       - if 0 is entered as the input frequency, the returned plot will
%          not have a specific frequency and/or frequency factor marked
%
%
% Inputs:
%        - Freq: frequency (Hz)
%        - Buoy: the specificed buoy (Ex: 'X01')
%        - Method: the method that should be used to determine the frequency
...
if A_omegaf ~= 0
    ...
    ann2 = annotation('textbox',[0.14,0.73,0.37,0.07],'string',...
        sprintf('Frequency Factor, f_w: %g\n',round(Af_w,3)),'fontsize',10);
